...
def preprocessing_fn(inputs):
    ...
    # We decode image first because we brings our data from tfds.
    ...
    image = tf.map_fn(_decode_image, inputs[IMAGE_KEY], fn_output_signature=tf.TensorSpec([None, None, 1], dtype=tf.float32))
    image = tft.scale_by_min_max(image, -0.5, 0.5)
    outputs[transformed_name(IMAGE_KEY)] = tf.reshape(image, [-1, 784])
...
def input_fn(file_pattern: List[str], data_accessor: DataAccessor, tf_transform_output: tft.TFTransformOutput, batch_size: int = 200) -> tf.data.Dataset:
...
def build_keras_model() -> tf.keras.Model:
    ...
    model.compile(loss="sparse_categorical_crossentropy", optimizer=tf.keras.optimizers.RMSprop(lr=0.0015), metrics=["sparse_categorical_accuracy"])
